# -*- coding: utf-8 -*-
"""
Created on Thu Mar  7 11:49:26 2024
...
class FiberSpan:
    ...
    def __init__(
        ...
        input_filter_power_function : float, optional
        ...
        output_atten_dB : float, optional
            ...
            misalignment. The default is 0.0.
        output_filter_power_function : float, optional
            Function describing spectrum of a filter to be applied to the
        ...
        if input_filter_power_function == None:
            def no_filter(freq):
        ...
        if output_filter_power_function == None:
            def no_filter(freq):
                return (1+0j)*np.ones_like(freq)
    ...
    def describe_fiber(self, destination=None):
        ...
        print(
            f"Output Amplification [dB] = {self.output_amp_dB} ", file=destination
        )

        print(
            f"Noise Factor [dB] = {self.output_noise_factor_dB} ", file=destination
        )

        print(
            f"Output Attenuation [dB] = {self.output_atten_dB} ", file=destination
        )
...
def SSFM(
    ...
    for fiber_index, fiber in enumerate(fiber_link.fiber_list):
        ...
        initial_pulse = np.copy(current_input_signal.amplitude)
        ...
        for z_step_index in range(fiber.numberOfSteps):
            ...
            if z_step_index == fiber.numberOfSteps - 1:
                ...
                outputAttenuationField_lin = np.sqrt(dB_to_lin(
                    fiber.output_atten_dB))
                output_filter_field_array = np.sqrt(fiber.output_filter_power_function(-f+fc))
...
def plot_first_and_last_pulse(ssfm_result_list: list[SSFMResult],
    ...
    **kwargs : TYPE
    ...
    ax.plot(t,np.polyval(p,t),'k--',label="Quadratic fit")
...
def make_chirp_gif_2x2(ssfm_result_list_list: list[list[SSFMResult]],
    ...
    timeFreq = ssfm_result_list_0[0].input_signal.timeFreq
    ...
    points_0 = np.array(
        [timeFreq.t * 1e12, get_power(matrix_0[len(zvals) - 1, Nmin:Nmax])],
        dtype=object
    ).T.reshape(-1, 1, 2)

    points_1 = np.array(
        [timeFreq.t * 1e12, get_power(matrix_1[len(zvals) - 1, Nmin:Nmax])],
        dtype=object
    ).T.reshape(-1, 1, 2)
    points_2 = np.array(
        [timeFreq.t * 1e12, get_power(matrix_2[len(zvals) - 1, Nmin:Nmax])],
        dtype=object
    ).T.reshape(-1, 1, 2)
    points_3 = np.array(
        [timeFreq.t * 1e12, get_power(matrix_3[len(zvals) - 1, Nmin:Nmax])],
        dtype=object
    ).T.reshape(-1, 1, 2)
    ...
    def update(i: int):
        ...
        init()  # Reset axes  {num:{1}.{5}}  np.round(,2)
        ax[0,0].set_title(f"Gaussian, z = {zvals[i]/scalingFactor:.2f}{letter}m")
        ax[0,1].set_title(f"Square")
        ax[1,0].set_title(f"Sech")
        ax[1,1].set_title(f"Triangle")


        # Make collection of points from pulse power
        points_0 = np.array(
            [timeFreq.t[Nmin:Nmax] * 1e12, get_power(matrix_0[i, Nmin:Nmax])],
            dtype=object
        ).T.reshape(-1, 1, 2)

        points_1 = np.array(
            [timeFreq.t[Nmin:Nmax] * 1e12, get_power(matrix_1[i, Nmin:Nmax])],
            dtype=object
        ).T.reshape(-1, 1, 2)

        points_2 = np.array(
            [timeFreq.t[Nmin:Nmax] * 1e12, get_power(matrix_2[i, Nmin:Nmax])],
            dtype=object
        ).T.reshape(-1, 1, 2)

        points_3 = np.array(
            [timeFreq.t[Nmin:Nmax] * 1e12, get_power(matrix_3[i, Nmin:Nmax])],
            dtype=object
        ).T.reshape(-1, 1, 2)
        ...
        # Activate norm function based on local chirp
        lc_0.set_array(get_chirp(timeFreq.t[Nmin:Nmax], matrix_0[i, Nmin:Nmax]) / 1e9)
        lc_1.set_array(get_chirp(timeFreq.t[Nmin:Nmax], matrix_1[i, Nmin:Nmax]) / 1e9)
        lc_2.set_array(get_chirp(timeFreq.t[Nmin:Nmax], matrix_2[i, Nmin:Nmax]) / 1e9)
        lc_3.set_array(get_chirp(timeFreq.t[Nmin:Nmax], matrix_3[i, Nmin:Nmax]) / 1e9)
